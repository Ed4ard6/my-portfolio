...
$contactLines = preg_split('/\r\n|\r|\n/', trim((string)($content['contact_body'] ?? '')));
$contactItems = [];
foreach ($contactLines as $line) {
    ...
    if ($typeLower === '' && filter_var($value, FILTER_VALIDATE_EMAIL)) {
        $typeLower = 'correo';
    }
    if ($typeLower === '' && str_contains($valueLower, 'linkedin.com')) {
        $typeLower = 'linkedin';
    }
    if ($typeLower === '' && str_contains($valueLower, 'github.com')) {
        $typeLower = 'github';
    }
    if ($typeLower === '' && str_contains($valueLower, 'wa.me')) {
        $typeLower = 'whatsapp';
    }
    ...
    $iconSvg = '<svg class="contact-icon" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2Z"/></svg>';
    if (in_array($typeLower, ['correo', 'email', 'mail'], true)) {
        $iconSvg = '<svg class="contact-icon" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M2 6a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v.35l-10 6.25L2 6.35V6Zm0 2.69V18a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8.69l-9.47 5.92a1 1 0 0 1-1.06 0L2 8.69Z"/></svg>';
    } elseif ($typeLower === 'linkedin') {
...
if (empty($contactItems)) {
    $contactItems = [
        ['href' => 'mailto:ej.machacon@gmail.com', 'label' => 'ej.machacon@gmail.com', 'icon' => '<svg class="contact-icon" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M2 6a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v.35l-10 6.25L2 6.35V6Zm0 2.69V18a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8.69l-9.47 5.92a1 1 0 0 1-1.06 0L2 8.69Z"/></svg>'],
    ];
}
?>
